...
shoot_bottom = Template(r"开火键.png", record_pos=(0.365, 0.132), resolution=(3040, 1440))
...
def manage():
    while True:
        for _ in range(5):
            ...
            if retry(shoot_bottom, 1, 5):
                if retry(eye_bottom, 1, 5):  # 开启视角
                    logger.warning("滑动视角")
                    swipe((1514, 294), (1511, 1394))
                    swipe((1514, 294), (1511, 1394))
                    logger.warning("查找空位")
                    retry(space_bottom, 1, 10)  # 选择空格
                    logger.warning("放置机枪")
                    retry(gun_bottom, 2, 5)  # 机枪
                    logger.warning("关闭视角")
                    retry(eye_bottom, 2, 5)  # 关闭视角
            else:
                retry(eye_bottom, 1, 5)  # 关闭视角
            if retry(update_bottom, 1, 3):  # 升级
                logger.warning("升级")
...
def game_start():
    logger.warning("开始游戏")
    retry(start_game_bottom, 5, 100)
    retry(shoot_bottom, 5, 5)
    # 如果不需要就删除这行 》
    while True:
    ...
    touch((908, 248))  # 删除装备
    logger.warning("选择装备")
    retry(gun_bottom, 5, 5)  # 机枪
    logger.warning("添加装备")
    retry(zhuangbei_bottom, 5, 5)  # 装备
    logger.warning("完成")
    retry(wan_cheng_bottom, 5, 5)  # 完成
    start_process()
    while True:
...
if __name__ == '__main__':
    while True:
        if retry(shoot_bottom, 3, 1):
            ...
            if retry(start_game_bottom, 3, 1):
                game_start()
            if retry(go_home, 3, 1):
                game_end()
